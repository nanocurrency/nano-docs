...
### Confirmation height currently processing
_version 19.0+_
...
Returns a list of pairs of peer IPv6:port and its node protocol network version    
Request:  
```
{  
  "action": "peers" 
...
### RPC callback
Send JSON POST requests with every confirmed block to callback server _http://callback_address:callback_port<callback_target>_ defined in [config.json](https://github.com/nanocurrency/nano-node/wiki/config.json). Callback target should include a leading slash. Block object is returned as a string [issue/749](https://github.com/nanocurrency/nano-node/issues/749) 
Sample:  
